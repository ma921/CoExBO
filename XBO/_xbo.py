...
class XBO:
    ...
    def set_models(self, X, Y, X_pairwise, y_pairwise):
        ...
        if self.learn_preference:
            ...
            if self.quadrature_method == "bq":
                prior_pref = BayesianQuadrature(model_pref, self.domain)
            else:
    ...
    def __call__(self, dataset_obj, dataset_duel, beta, gamma, sigma=None):
        ...
        return result, dataset_obj_new, dataset_duel_new
...
class XBOwithHuman(HumanFeedback):
    def __init__(
        self, 
        domain, 
        true_function, 
    ...
    def posthoc_evaluation(self, X, Y, X_pairwise, y_pairwise, y_pairwise_unsure, X_pairwise_next, y_pairwise_next):
        model = set_and_fit_rbf_model(X, Y)
    ...
    def __call__(self, dataset_obj, dataset_duel, beta, gamma):
        ...
        # 2. evaluate the process
        best_obs = Y.max().item()
        pi, total_pi_mean = self.posthoc_evaluation(X, Y, X_pairwise, y_pairwise, y_pairwise_unsure, X_pairwise_next, y_pairwise_next)
