...
from GPSHAP.explanation_algorithms.BayesGPSHAP import BayesGPSHAP
...
def global_explanation_plot(feature_names: list[str],
    ...
    global_importance = global_importance.sort_values("std")
    plt.barh(global_importance.index, global_importance["std"], alpha=.5, color="blue")
    plt.xlabel("Uncertainty on Stochastic Shapley Values")
    plt.title("Global uncertainty")
    plt.ylabel("")
    plt.tight_layout()
    plt.show()
    """
